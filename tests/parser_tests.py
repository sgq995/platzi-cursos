...
class ParserTest(TestCase):
  ...
  def test_prefix_expression(self) -> None:
    ...
    self._test_program_statements(parser, program, exprected_statement_count=2)
  ...
  def _test_program_statements(self,
                              parser: Parser,
                              program: Program,
                              exprected_statement_count: int = 1) -> None:
    if parser.errors:
      print(parser.errors)

    self.assertEquals(len(parser.errors), 0)
    self.assertEquals(len(program.statements), exprected_statement_count)
